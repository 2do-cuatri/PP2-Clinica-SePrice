...
class BorradorSuppliesActivity : AppCompatActivity() {
...
/*
        ...
        val restockButton = findViewById<Button>(R.id.restock_btn)
        restockButton.setOnClickListener {
            stockInsumos = intArrayOf(50, 50, 50, 50)
            updateStock()
        }

        val useSuppliesBtn = findViewById<Button>(R.id.use_selected_btn)
        useSuppliesBtn.setOnClickListener {
            val error = useSelectedInsumos()
            if (error == null) {
                updateStock()
            } else {
    ...
    private fun updateStock() {
        ...
        val insumo_a_text: TextView = findViewById(R.id.cant_insumo_a_txt)
        val insumo_b_text: TextView = findViewById(R.id.cant_insumo_b_txt)
        val insumo_d_text: TextView = findViewById(R.id.cant_insumo_d_txt)
        val insumo_c_text: TextView = findViewById(R.id.cant_insumo_c_txt)
    ...
    private fun useSelectedInsumos(): Error? {
        var error: Error? = null
        val nuevoStock = intArrayOf(0, 0, 0, 0)
